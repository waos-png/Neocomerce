...
export default function Paso2Detalles({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  const { producto, setProducto } = useProducto();
  const [error, setError] = useState('');

  const validarYContinuar = () => {
    if (!producto.nombre || !producto.descripcion || !producto.estado) {
  ...
  return (
    // Elimina el espacio inferior y el posible div externo que genera la sombra/caja extra
    <div className="bg-white p-8 rounded-3xl shadow-2xl space-y-8 text-[#18122B] relative overflow-hidden font-['Poppins',_ui-sans-serif]">
      {/* Glow border effect */}
      {/* Elimina o comenta el siguiente div si genera la sombra/caja extra */}
      {/* <div className="absolute -inset-1 rounded-3xl pointer-events-none z-0" style={{ boxShadow: '0 0 60px 10px #d946ef33' }} /> */}
      ...
      <div className="space-y-5 z-10 relative">
        <div>
          <input
            ...
            onChange={e => setProducto({ nombre: e.target.value })}
        ...
        <div>
          <textarea
            ...
            onChange={e => setProducto({ descripcion: e.target.value })}
            className="w-full p-4 rounded-xl bg-[#f8f5fc] text-[#18122B] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-fuchsia-500 border border-fuchsia-200 focus:border-fuchsia-400 transition shadow-md min-h-[90px] resize-none font-['Poppins',_ui-sans-serif]"
          />
        </div>
        <div className="relative">
          <select
            value={producto.estado}
            onChange={e => setProducto({ estado: e.target.value as 'nuevo' | 'usado' })}
            className="w-full p-4 pr-10 rounded-xl bg-[#f8f5fc] text-[#18122B] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-fuchsia-500 border border-fuchsia-200 focus:border-fuchsia-400 transition shadow-md cursor-pointer font-['Poppins',_ui-sans-serif]"
          >
            <option value="" className="text-gray-400 bg-[#f8f5fc]">Seleccionar estado</option>
            <option value="nuevo" className="text-[#18122B] bg-[#f8f5fc]">Nuevo</option>
            <option value="usado" className="text-[#18122B] bg-[#f8f5fc]">Usado</option>
          </select>
          {/* Custom arrow icon */}
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-fuchsia-400 pointer-events-none">
